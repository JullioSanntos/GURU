...
namespace GURU.Model
{
    ...
    public class MainModel : BindableBase, IDisposable
    {
        ...
        public bool IsValid
        {
            get { return ValidationErrors.Any() == false; }
            //get
            //{
        ...
        public List<string> ValidationErrors
        {
            get
            {
                ...
                _validationErrors.AddRange(ValidationRule4());
                _validationErrors.AddRange(ValidationRule5());
        ...
        public List<string> ValidationRule1()
        {
            ...
            if (this.ComposedElements?.Any(ce => ce.HasGradedTypes) == false)
                errorMessages.Add($"01: Graded Elements (Magnitude or Severity != 0) were not found");
        ...
        public List<string> ValidationRule2()
        {
            ...
            if (this.Interfaces?.Any() == false)
                errorMessages.Add($"02: No Interfaces were found");
        ...
        public List<string> ValidationRule3()
        {
            ...
            //Reject submit if there are Elements that do not have an Element Type
            var ElemsWithmissingTypes = this.ComposedElements?.Where(e => e.ElementType == null || e.ElementType.Name == null).ToList();
            if (ElemsWithmissingTypes.Any())
            {
                errorMessages.Add($"03: there are Elements that do not have an Element Type:");
        ...
        public List<string> ValidationRule4()
        {
            ...
            //Reject submit if there are invalid Interfaces
            var InvalidInterface = this.Interfaces?.Where(i => i.Element1 == null || i.Element2 == null || i.GradedFailureModesTypesList.Any() == false).ToList();
            if (InvalidInterface.Any())
            {
                errorMessages.Add($"03: there are invalid Interfaces that have missing Failure Type or Elements:");
                InvalidInterface.ForEach(i => errorMessages.Add($"     Interface Name: {i.Name}"));
            }


            return errorMessages;
        }
        public List<string> ValidationRule5()
        {
            var errorMessages = new List<string>();

            // Get all Element's that participate in Interfaces using their Identity property
            var allInterfacedElements = this.Interfaces?.Select(e1 => e1.Element1).Union(this.Interfaces.Select(e2 => e2.Element2))
                .Where(e => e != null && string.IsNullOrEmpty(e.Name) == false && String.IsNullOrEmpty(e.Id) == false && e.ElementType != null)?.ToList();

            if (allInterfacedElements?.Count != ComposedElements.Where(ce => ce.HasGradedTypes).Count()) {
                errorMessages.Add($"04: There are Elements that do not participate in Interfaces or are defective:");
